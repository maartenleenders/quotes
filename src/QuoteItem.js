import { ReactComponent as QuoteBackground } from "./quote-background.svg";
...
export default function QuoteItem( { conversation } ) {
	...
	if ( conversation.quote.length > 1 ) {
		formattedQuote = conversation.quote.map( ( { sentence, author }, index ) => {
			return <p className="text-black" key={ `quote-${ author }-${ index }`} >{ sentence }<span className="text-gray-600 text-base">{ `~ ${author}` }</span></p>
		} );
	} else {
				...
				<footer className="mt-8">
				  <div className="flex">
					<div className="ml-4 lg:ml-0">
					  <div className="text-base font-medium text-gray-900">{ conversation.quote.map( ( { author } ) => author ).join( " & ") }</div>
